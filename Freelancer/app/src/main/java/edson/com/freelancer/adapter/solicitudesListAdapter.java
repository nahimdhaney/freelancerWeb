...
public class solicitudesListAdapter extends BaseAdapter {
    ...
    public solicitudesListAdapter(Context context, List<Solicitud> items) {
    ...
    public long getItemId(int i) {
        return items.get(i).getId_solicitud();
    }

    @Override
    public View getView(int i, View view, ViewGroup viewGroup) {
        if(view == null){
            view = LayoutInflater.from(context).inflate(R.layout.layou_list_solicitud, viewGroup, false);
        }
        TextView tv_NombreList = (TextView)view.findViewById(R.id.tv_NombreList);
        Solicitud item = items.get(i);
        tv_NombreList.setText(item.getFreelancer());
